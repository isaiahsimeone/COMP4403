 ...
 * DoStatement -> KW_DO DoBranch { SEPARATOR DoBranch } KW_DO
...
public class Parser {
    ...
    /**
     * Set of tokens that may start a DoBranch.
     */
    private final static TokenSet DO_BRANCH_START_SET =
            CONDITION_START_SET;
    ...
    /**
     * Methods for parsing Lists of ExpNode's
     */
    private final ParseMethod<List<ExpNode>> expList =
    ...
    private List<ExpNode> parseConditionList(TokenSet recoverSet) {
        return expList.parse("Condition List", CONDITION_START_SET, recoverSet,
                () -> {
                    ...
                    /* Parse the first condition on the RHS */
                    ExpNode condition =
                            parseCondition(recoverSet.union(Token.COMMA));
                    conditions.add(condition);
                    /* Match subsequent ExpNode's contained on the RHS of the
                     * Assignment statement */
                    while (tokens.isMatch(Token.COMMA)) {
                        tokens.match(Token.COMMA); /* Cannot Fail */
                        condition = parseCondition(recoverSet);
                        conditions.add(condition);
                    }
                    return conditions;
                });
    ...
    private List<ExpNode> parseLValueList(TokenSet recoverSet) {
        return expList.parse("LValue List", LVALUE_START_SET, recoverSet,
                () -> {
                    ...
                    /* Parse the first LValue on the LHS. */
                    ExpNode lvalue = parseLValue(recoverSet.union(Token.COMMA));
                    lvalues.add(lvalue);

                    /* Match subsequent ExpNode's contained on the LHS of the
                     * Assignment statement */
                    while (tokens.isMatch(Token.COMMA)) {
                        tokens.match(Token.COMMA); /* Cannot fail. */
                        lvalue = parseCondition(recoverSet.union(Token.COMMA));
                        lvalues.add(lvalue);
                    }
                    return lvalues;
                });
    ...
    private StatementNode.AssignmentNode parseAssignment(TokenSet recoverSet) {
        return assign.parse("MDAssignment", LVALUE_START_SET, recoverSet,
                () -> {
                    /* Recovery set for parsing LValueList */
                    TokenSet lvalueRecover = recoverSet.union(Token.ASSIGN,
                            Token.EQUALS, Token.COMMA);
                    /* Recovery set for parsing ConditionList */
                    TokenSet conditionRecover = recoverSet.union(Token.COMMA);

                    List<ExpNode> left, right;
                    /* Parse LValueList from the left side of the assignment statement. */
                    left = parseLValueList(lvalueRecover);
                    Location loc = tokens.getLocation();
                    tokens.match(Token.ASSIGN, CONDITION_START_SET);
                    /* Parse the ConditionList from the right side of the assignment statement. */
                    right = parseConditionList(conditionRecover);
                    /* If there is a LHS-RHS mismatch, report that error */
                    if (right == null || left.size() != right.size()) {
                        errors.error("number of variables does not match number " +
                                "of expressions in assignment", loc);
                    }
                    /* If there are more LHS values than RHS values, add individual error
                     * Nodes to the AssignmentNode */
                    if (right != null && left.size() > right.size()) {
                        while (left.size() > right.size()) {
                            right.add(new ExpNode.ErrorNode(tokens.getLocation()));
                        }
                    }
    ...
    private StatementNode parseDoStatement(TokenSet recoverSet) {
        return stmt.parse("Do Statement", Token.KW_DO, recoverSet,
                () -> {
                    /* A list of the DoBranches in the DoStatement being
                     * parsed. */
                    LinkedList<StatementNode.DoBranch> branches = new LinkedList<>();

                    Location loc = tokens.getLocation();
                    tokens.match(Token.KW_DO);  /* Cannot fail. */
                    /* Parse the first Do Branch of the statement */
                    branches.add(parseDoBranch(recoverSet.union(Token.KW_OD,
                            Token.SEPARATOR)));
                    /* Each subsequent SEPARATOR token initiates a new do
                     * branch, which should be parsed. */
                    while (tokens.isMatch(Token.SEPARATOR)) {
                        tokens.match(Token.SEPARATOR); /* Cannot fail. */
                        branches.add(parseDoBranch(recoverSet.union(Token.KW_OD,
                                Token.SEPARATOR)));
                    }
                    /* Match the 'od' Token which ends the statement. We expect
                     * another statement to follow. */
    ...
    /**
     * Used for parsing the Do Branch portion of Do Statements
     */
    private final ParseMethod<StatementNode.DoBranch> doBranch =
            new ParseMethod<>( (Location loc) -> null );

    /**
     * Rule: DoBranch -> Condition KW_THEN StatementList [KW_EXIT]
     */
    private StatementNode.DoBranch parseDoBranch(TokenSet recoverSet) {
        return doBranch.parse("Do Branch", DO_BRANCH_START_SET, recoverSet,
                () -> {
                    /* Leading condition of the DoBranch being parsed. */
                    ExpNode condition;
                    /* The StatementList in the DoBranch. */
                    StatementNode stmt;
                    /* True iff the DoBranch is followed by KW_EXIT,
                     * False otherwise. */
                    boolean hasExit = false;

                    Location loc = tokens.getLocation();
                    /* Parse the condition node which leads the DoBranch. */
                    condition = parseCondition(recoverSet.union(Token.KW_THEN));
                    /* Parse the KW_THEN token which we expect to be followed
                     * by a statementList or the KW_EXIT token (if for some
                     * reason there are no statements. */
                    tokens.match(Token.KW_THEN, STATEMENT_START_SET);
                    /* Parse the statementList which follows the KW_THEN token. */
                    stmt = parseStatementList(recoverSet);

                    /* If KW_EXIT is present, parse it. */
                    if (tokens.isMatch(Token.KW_EXIT)) {
                        tokens.match(Token.KW_EXIT); /* Cannot fail. */
                        hasExit = true;
                    }

                    return new StatementNode.DoBranch(loc, condition, stmt, hasExit);
                });
    }
    ...
    private StatementNode parseSkipStatement(TokenSet recoverSet) {
        return stmt.parse("Call Statement", Token.KW_SKIP, recoverSet,
                () -> {
